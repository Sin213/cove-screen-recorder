//! T-055 — QSV Windows probe shape tests.
...
mod windows_tests {
    ...
    use cove_replay_engine::encoder::backends::qsv::QsvBackend;

    #[tokio::test]
    async fn qsv_windows_probe_returns_unavailable_not_implemented() {
        std::env::remove_var("COVE_QSV_FORCE_UNAVAILABLE");
        let backend = QsvBackend::new();
    ...
    #[tokio::test]
    async fn qsv_windows_teardown_returns_ok() {
        let mut backend = QsvBackend::new();
...
#[cfg(not(windows))]
#[test]
fn qsv_windows_tests_only_run_on_windows() {}
