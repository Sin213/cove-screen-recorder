...
- `.story/notes/N-003.json` — **created**. Full design record: boundary, trait surface, portal flow, DMA-BUF/SHM negotiation, format/framerate/cursor handling, dynamic-resolution behaviour, failure surface, diagnostics, deferred items, validation matrix.
- `.story/project-state.md` — **modified**. New section `## v2.0.0 PipeWire capture backend boundary (T-004, 2026-05-13)` inserted between the T-003 decision section and the Open issue triage table.
- `.story/tickets/T-004.json` — **modified**. Status `open` → `inprogress` → `complete`. Description rewritten with the locked boundary, the issue absorption commitments, and the deferred-item list.
- `.story/handovers/2026-05-13-04-t-004-pipewire-boundary.md` — **created**. This file.
...
If a second planning tab is available, T-008 (UI ↔ engine integration) can also start in parallel — it now has the full Electron-facing method and event surface from this design. T-006 should wait for T-005 (segment buffer needs the encoder's accepted format set and timestamp semantics).
...
- `storybloq_ticket_update T-004 status=inprogress` (start) → `status=complete` with rewritten description (end).
- `storybloq_note_create` → N-003 (PipeWire capture backend boundary design record).
